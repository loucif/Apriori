...
public class AprioriFrequentItemsetGenerator<I> {
    ...
    public FrequentItemsetData<I> generate(List<Set<I>> transactionList, double minimumSupport) {
        ...
        System.err.println("Number Of Transaction : " + transactionList.size());
        System.err.println("The List Transactions : " + transactionList);
        ...
        do {
            ...
            for (Set<I> transaction : transactionList) {
                ...
                for (Set<I> itemset : candidateList2) supportCountMap.put(itemset, supportCountMap.getOrDefault(itemset, 0) + 1);
            }
    ...
    private List<Set<I>> extractFrequentItemsets(Map<Integer, List<Set<I>>> map) {
        ...
        for (List<Set<I>> itemsetList : map.values()) ret.addAll(itemsetList);
    ...
    private List<Set<I>> subset(List<Set<I>> candidateList, Set<I> transaction) {
        ...
        for (Set<I> candidate : candidateList) if (transaction.containsAll(candidate)) ret.add(candidate);
    ...
    private Set<I> tryMergeItemsets(List<I> itemset1, List<I> itemset2) {
        ...
        for (int i = 0; i < length - 1; ++i) if (!itemset1.get(i).equals(itemset2.get(i))) return null;

        if (itemset1.get(length - 1).equals(itemset2.get(length - 1))) return null;

        Set<I> ret = new HashSet<>(length + 1);

        for (int i = 0; i < length - 1; ++i) ret.add(itemset1.get(i));
    ...
    private List<Set<I>> findFrequentItems(List<Set<I>> itemsetList, Map<Set<I>, Integer> supportCountMap, double minimumSupport) {
        ...
        for (Set<I> itemset : itemsetList) {
            for (I item : itemset) {
                ...
                tmp.add(item);
                System.out.println("list itemos :" + item);
                if (supportCountMap.containsKey(tmp)) supportCountMap.put(tmp, supportCountMap.get(tmp) + 1);
                else supportCountMap.put(tmp, 1);
    ...
    private void checkSupport(double support) {
        if (Double.isNaN(support)) throw new IllegalArgumentException("The input support is NaN.");

        if (support > 1.0) throw new IllegalArgumentException("The input support is too large: " + support + ", " + "should be at most 1.0");

        if (support < 0.0) throw new IllegalArgumentException("The input support is too small: " + support + ", " + "should be at least 0.0");
    }
}